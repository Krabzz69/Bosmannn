...
const API = null;
let tenants = [
  { id: 'demo-restaurant', name: 'Demo Restaurant', theme_color: '#ff6b35' }
];
let selectedTenant = null;
let products = [
  { id: 1, name: 'Burger', description: 'Delicious beef burger', price: 250, category: 'Main', image_url: '' },
  { id: 2, name: 'Pizza', description: 'Cheesy pizza', price: 450, category: 'Main', image_url: '' },
  { id: 3, name: 'Coke', description: 'Cold drink', price: 80, category: 'Drinks', image_url: '' },
  { id: 4, name: 'Fries', description: 'Crispy fries', price: 120, category: 'Sides', image_url: '' }
];
let cart = [];
...
function renderRestaurants() {
    $('restaurantsGrid').innerHTML = tenants.map(t => `
        <div class="restaurant-card" data-slug="${esc(t.id)}" data-id="${t.id}" data-name="${esc(t.name)}" data-color="${esc(t.theme_color || '#D32F2F')}">
            <div class="restaurant-card-logo" style="background: linear-gradient(135deg, ${esc(t.theme_color || '#D32F2F')}, ${esc('#1565C0')})">
                ${esc(t.name.charAt(0))}
            </div>
            <div class="restaurant-card-body">
                <div class="restaurant-card-name">${esc(t.name)}</div>
                <div class="restaurant-card-address">Demo Restaurant</div>
            </div>
        </div>
    `).join('');
...
async function checkout() {
    ...
    setTimeout(() => {
        ...
        $('successInvoice').textContent = `Invoice: ${invoiceNum}`;
